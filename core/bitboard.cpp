...
void BitBoard::generateMoves(MoveList & moveList) const
{
  ...
  generateSlideAttacks(queens, mNorthWestAttacks, friends, mRotateR45, a8h1Shifts, mNorthWestMasks, moveList);
}
...
void BitBoard::testRotations()
{
  ...
  // Do move
  mPiece[All] ^= fromTo;
  rotated &= mInvMask[rotateR90Matrix[fromSq]];
  rotated |= mMask[rotateR90Matrix[toSq]];
...
void BitBoard::writeOccupancy(uint occupancy, std::ostream & output) const
{
  for (int i = 0; i < 8; i++) {
    if (occupancy & (1 << i))
      output << 'X';
