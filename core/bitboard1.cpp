 ...
 */
inline uchar BitBoard1::bitScanForward(U64 bb) const
{
...
uchar BitBoard1::bitScanForwardPopCount(U64 bb) const
{
  assert (bb != 0);
  return popCount( (bb & -bb) - 1 );
}

void BitBoard1::generateBlackPawnMoves(MoveList &moveList) const
{
  U64 bbMove = 0;
...
void BitBoard1::generateCastlingMoves(MoveList & moveList) const
{
...
void BitBoard1::generateMoves(MoveList & moveList) const
{
...
void BitBoard1::generateJumpAttacks(U64 piece, const U64 * attacks, U64 friends, MoveList & moveList) const
{
  while (piece) {
...
void BitBoard1::generatePawnAttacks(U64 piece, const U64 *attacks, U64 enemies, MoveList &moveList) const
{
  while (piece) {
    uchar fromSq = bitScanForward(piece);
    U64 attack = attacks[fromSq];
...
void BitBoard1::generateSlideAttacks(U64 piece, const U64 attacks[][256], U64 friends, U64 pieces, const uint * shifts, const uint * masks, MoveList & moveList) const
{
  while (piece) {
    uchar fromSq = bitScanForward(piece);
...
void BitBoard1::generateWhitePawnMoves(MoveList &moveList) const
{
  U64 bbMove = 0;
...
void BitBoard1::initBishopAttacks()
{
  // North east attacks
...
void BitBoard1::initBlackPawnAttacks()
{
...
void BitBoard1::initKingAttacks()
{
...
void BitBoard1::initKnightAttacks()
{
...
void BitBoard1::initRookAttacks()
{
  // Rank attacks
...
void BitBoard1::initWhitePawnAttacks()
{
...
bool BitBoard1::isCellAttacked(uchar row, uchar col, Color attackingColor) const
{
  return isCellAttacked(getIndex(row, col), attackingColor);
}

bool BitBoard1::isCellAttacked(uchar index, Color attackingColor) const
{
...
void BitBoard1::makeMove(const Move & move)
{
  ...
  Undo undo;
  undo.rotate90 = mRotateRight90Pieces;
  undo.rotateL45 = mRotateLeft45Pieces;
  undo.rotateR45 = mRotateRight45Pieces;
  mUndo[mUndoPos++] = undo;
  ...
  U64 mask[64], imask[64];
  for (int i = 0; i < 64; i++) {
    mask[i] = (ONE << i);
    imask[i] = ~mask[i];
  }

  U64 mr90[64], imr90[64];
  for (int i = 0; i < 64; i++) {
    mr90[i] = mask[rotateL90Matrix[i]];
    imr90[i] = imask[rotateL90Matrix[i]];
  }

  //writeBitBoard(mRotateRight90Pieces, std::cout);
  //writeBitBoard(ZERO << rotateR90Matrix[fromSquare], std::cout);
  //mRotateRight90Pieces &= imr90[fromSquare];
  //mRotateRight90Pieces |= mr90[toSquare];
  //mRotateRight45Pieces &= (ZERO << rotateR45Matrix[fromSquare]);
  //mRotateRight45Pieces |= (ONE << rotateR45Matrix[toSquare]);
  //mRotateLeft45Pieces &= (ZERO << rotateL45Matrix[fromSquare]);
  //mRotateLeft45Pieces |= (ONE << rotateL45Matrix[toSquare]);
  //writeBitBoard(mRotateRight90Pieces, std::cout);
...
uchar BitBoard1::popCount (U64 x) const
{
  uchar count = 0;
  while (x) {
    count++;
...
void BitBoard1::pushBitBoardMoves(U64 bb, uchar fromSq, MoveList & moveList) const
{
  while (bb) {
...
void BitBoard1::pushMove(uchar from, uchar to, Type piece, Type capture, Type promote, Move::Type type, MoveList & moveList) const
{
...
PieceType BitBoard1::pieceType(uchar row, uchar col) const
{
...
BitBoard1::U64 BitBoard1::invRotateL45(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
BitBoard1::U64 BitBoard1::invRotateR45(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
BitBoard1::U64 BitBoard1::rotateL45(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
BitBoard1::U64 BitBoard1::rotateL90(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
BitBoard1::U64 BitBoard1::rotateR45(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
BitBoard1::U64 BitBoard1::rotateR90(U64 bb) const
{
  U64 symmBB = 0;
  for (uint index = 0; index < 64; index++)
...
void BitBoard1::setPosition(const std::string & fenString)
{
  initBoard();
...
void BitBoard1::unmakeMove(const Move & move)
{
  // Get move information
  Color side = sideToMove();
  ...
  Undo undo = mUndo[--mUndoPos];
  mRotateRight90Pieces = undo.rotate90;
  mRotateLeft45Pieces = undo.rotateL45;
  mRotateRight45Pieces = undo.rotateR45;
...
void BitBoard1::updateAggregateBitBoards()
{
...
void BitBoard1::writeBitBoard(U64 bitboard, std::ostream & output) const
{
  for (int i = 7; i >= 0; i--) {
    std::cout << i+1 << " ";
    for (int j = 0; j < 8; j++) {
...
void BitBoard1::writeOccupancy(uint occupancy, bool flip) const
{
